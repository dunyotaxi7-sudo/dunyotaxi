...
const YandexLiveMap = dynamic(
  () => import("./YandexLiveMap").then((m) => m.YandexLiveMap),
  { ssr: false },
);

/**
 * Renders the live map. When no map key is configured we degrade to a simple
 * list of markers so the page still works in development — this fallback is
 * intentionally provider-neutral too.
 */
  ...
  return (
    <div className="h-full overflow-auto p-4">
      <div className="mb-3 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs text-amber-800">
        Xarita kaliti sozlanmagan (<code>NEXT_PUBLIC_YANDEX_MAPS_KEY</code>).
